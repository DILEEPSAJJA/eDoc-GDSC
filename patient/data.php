...
<head>
   ...
   /* table {
    ...
    th {
      background-color: rgb(7, 96, 199);
      color: white;
    }
    tbody tr:hover {
      background-color: rgb(141, 183, 239);
    }*/
  </style>
</head>
<body>
...
<div>
    <table style="border-radius: 10px;padding: 20px;margin: 5px;">
                <center>
      <h2>DRUG DATA</h2>
    <input type="text" style="width: 400px; height: 50px; border: 1px solid black; border-radius: 5px; margin: 10px;" id="searchInput" placeholder="Search For a Drug Name" height="30px" width="100px"></center>

  <table style="padding: 9px; margin: 6px 0;">
    <thead>
      <tr style="background-color: #006dd3">
        <th>id</th>
        <th>name</th>
        <th>substitute0</th>
        <th>substitute1</th>
  ...
  <script type="text/javascript">
    fetch("./csvjson.json")
      ...
      .then(data => {
        ...
        data.forEach(item => {
          ...
          row.insertCell(2).textContent = item.substitute0;
          row.insertCell(3).textContent = item.substitute1;
          row.insertCell(4).textContent = item.sideEffect0;
          row.insertCell(5).textContent = item.sideEffect1;
          row.insertCell(6).textContent = item.sideEffect2;
          row.insertCell(7).textContent = item.use0;
          row.insertCell(8).textContent = item.use1;
          row.insertCell(9).textContent = item.use2;
        });
      })
      .catch(err => {
        console.log(err);
    ...
    <?php 
    if($_GET){
        if (isset($_GET["action"])) { // Check if action parameter is set
